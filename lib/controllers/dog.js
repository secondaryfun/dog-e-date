...
module.exports = {
  ...
  addLike: (req, res) => {
    let id = req.params.id
    Dog.findByIdAndUpdate(id, { $push: { likes: req.body.userId } })
      .then(dog => res.json(dog))
      .catch(error => console.log(error))
  },
  addIgnore: (req, res) => {
    let id = req.params.id
    Dog.findByIdAndUpdate(id, req.body, { $push: { ignores: id } }).then(dog => res.json(dog)).catch(error => console.log(error))
  },
}

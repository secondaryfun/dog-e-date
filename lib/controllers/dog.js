...
module.exports = {
  ...
  addLike: (req, res) => {
    let id = req.params.id
    Dog.findByIdAndUpdate(id, { $push: { likes: friend } }).then(dog => res.json(dog)).catch(error => console.log(error))
  },
  addIgnore: (req, res) => {
    let id = req.params.id
    Dog.findByIdAndUpdate(id, { $push: { ignores: friend } }).then(dog => res.json(dog)).catch(error => console.log(error))
  },
}
